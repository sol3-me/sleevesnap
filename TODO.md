...
- [x] Landing follow-up (b): About page (2026-07-23) — new `/about` route (public router only, same as `/login`/`/signup`), linked from an "About" entry in the landing header nav (next to Sign in/Scan, not the footer). Shares the landing page's cover-wall background (extracted the fetch/tile logic into `hooks/useLandingWall.ts` + `components/LandingWallBackground.tsx` so both views use the same thing); logo/title in the page header now links back home. Covers the record-store use case ("can't remember if I already own this") plus the ownership-ethos paragraph, and explains the project is open source on GitHub, designed/built by sol3uk, with links to both. No em dashes in the copy per Ben's ask. Note: the original cut paragraph text was never committed anywhere (not in the repo, not in this TODO), so the copy is freshly drafted in the established brand voice (see `landing-page-design.md`) — Ben should review/edit it, it's not a recovered original.
