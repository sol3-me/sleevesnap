...
**Update (July 2026, `feature/firebase-auth`):** the real thing landed. Firebase Authentication (Google + GitHub popup sign-in, email/password with verification), ID-token verification on every API route via `firebase-admin` (project id only, no service account), and per-user scoping of collection/scan/scan-history data in SQLite. Pre-auth test data was left unowned rather than building a claim mechanism for it — a one-time `LEGACY_CLAIM_EMAIL` adoption path was prototyped but pulled back out: it required a verified email (Firebase tokens don't refresh their `email_verified` claim on their own after the user clicks the verification link, so the flow needed an awkward sign-out/sign-in to actually work) and the data it would have recovered was only testing data anyway. Spotify sign-in was assessed and deferred (not OIDC-compliant; needs a custom-token flow). Still open from that work: a public demo/guest mode with local-first storage and transfer-on-signup (phase 2), signed URLs for `/covers` (currently public because `<img>` can't send bearer tokens), and per-account vision-quota overrides (the `VISION_ADMIN_KEY` TODO in `.env.example`).
...
> **Design pass shipped (`b66af56`, July 2026):** a full hand-rolled visual modernization landed ahead of the library adoptions below — layered dark palette (vinyl-950…600) with white/alpha borders, self-hosted Inter Variable (partially covers §6.4; a display face for headings is still open), global `:focus-visible` accent ring (covers the focus item in §7), glassy blurred mobile bottom nav with safe-area support (`pb-safe` + `viewport-fit=cover`), a new brand mark, modernized cards/inputs/buttons/badges throughout, skeleton loading cards on Discover (covers §4.2), and filter chips that glow with a count badge when narrowing (covers the visibility half of §2.9 — the one-click reset is still open). §6.1 (shadcn/ui), §6.3 (lucide-react), §6.5 (motion), and §6.6 (cmdk) remain as written and are still the right next steps; the new styling was deliberately kept to plain Tailwind classes so shadcn components can be dropped in without unwinding anything.
