...
const routeApi = getRouteApi('/labels/$labelId');
...
export function LabelDetailView() {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to load label releases. Please try again.');
    } finally {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to load more label releases. Please keep scrolling to retry.');
    } finally {
      loadMoreInFlightRef.current = false;
      setIsLoadingMore(false);
    }
  ...
  const openLabelDetailFromCard = useCallback(async (nameFromCard: string, labelIdFromCard?: string) => {
    const trimmed = nameFromCard.trim();
    if (!trimmed) return;
    ...
    try {
      if (labelIdFromCard) {
        await navigate({
          to: '/labels/$labelId',
          params: { labelId: labelIdFromCard },
  ...
  return (
    <div className="p-4 md:p-8 pb-28 md:pb-24">
      ...
      <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-white">{labelName}</h2>
      <p className="text-xs text-gray-500 mt-1">Label ID: {labelId}</p>
      ...
      {isSearching ? (
        <div className="space-y-3 mt-4" aria-label="Loading label releases" role="status">
          ...
          {groupedSections.map((section) => (
            <section key={section.key}>
              ...
              <ReleaseGroupResultsList
                ...
                labelContext={{ id: labelId, name: labelName }}
                onLabelNameClick={(nameFromCard, labelIdFromCard) => {
                  void openLabelDetailFromCard(nameFromCard, labelIdFromCard);
                }}
              />
            </section>
          ))}
      ...
      {!isSearching && searchPage.total === 0 && (
        <div className="text-center text-gray-500 mt-10">No release groups found for this label.</div>
      )}
