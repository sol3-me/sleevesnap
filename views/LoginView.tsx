...
export type EmailMode = 'sign-in' | 'sign-up';
...
type LoginViewProps = {
  /** Which email form is showing — controlled by the /login or /signup route. */
  mode: EmailMode;
  /** Called to switch forms; the caller navigates to the other route. */
  onModeChange: (next: EmailMode) => void;
  /** Returns to the landing page. */
  onBack: () => void;
};

export function LoginView({ mode, onModeChange, onBack }: LoginViewProps) {
  ...
  // Local UI state (typed input, validation errors) shouldn't survive a mode
  // switch — the fields are hidden while the sign-in/sign-up card swaps.
  useEffect(() => {
    setFieldErrors({});
  }, [mode]);
